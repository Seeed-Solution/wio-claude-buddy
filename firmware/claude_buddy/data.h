...
// Plan-usage gauges, fed by the unified BLE bridge's claude-usage v:1 JSON
// (separate from the Hardware Buddy session heartbeat). reset_s counts down
// locally between pushes.
struct UsageState {
  bool have;
  int  sessPct, weekPct;
  long sessResetS, weekResetS;
  char weekLabel[16];
  long todayTokens;
};
static UsageState _usage = { false, 0, 0, 0, 0, "", 0 };
inline const UsageState& dataUsage() { return _usage; }
inline void dataUsageTick() {            // 1Hz local countdown
  if (_usage.sessResetS > 0) _usage.sessResetS--;
  if (_usage.weekResetS > 0) _usage.weekResetS--;
}
...
static void _applyJson(const char* line, TamaState* out) {
  ...
  if ((int)(doc["v"] | 0) == 1) {
    ...
    _usage.sessPct    = su["pct"]     | 0;
    _usage.sessResetS = su["reset_s"] | 0;
    _usage.weekPct    = wu["pct"]     | 0;
    _usage.weekResetS = wu["reset_s"] | 0;
    strlcpy(_usage.weekLabel, wu["reset_label"] | "", sizeof(_usage.weekLabel));
    _usage.todayTokens = tu["tokens"] | 0;
    _usage.have = true;
    _lastLiveMs = millis();
    return;
  }
  ...
  if (!t.isNull() && t.size() == 2) {
    ...
    rtcSetTime(&tm);
    rtcSetDate(&dt);
    // (upstream's clock-face re-read latch is dropped — the clock screen is
    // deferred in this port; the soft RTC just stores the synced time.)
    _rtcValid = true;
    _lastLiveMs = millis();
    return;
  }
